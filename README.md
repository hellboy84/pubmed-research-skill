...
- PubMedで実際に実行された検索式やその結果に基づいて動作します。取得したデータに基づいて動作することで、事前学習の知識やモデルの確率論的な挙動に由来するタイプのハルシネーションを抑制します。
...
- [生成されたレポート](SAMPLE/tirzepatide_t2dm_obesity_review.md) : 引用文献付。使用した検索式とヒット件数なども収録
- [生成された文献リスト](SAMPLE/tirzepatide_t2dm_obesity_list.csv) : 検索でヒットした文献のリスト
...
| `mesh` | MeSH（統制語彙）の検索 |
| `spell` | 検索語のスペルチェック |
| `to_csv` | 検索結果を CSV に書き出し（UTF-8 BOM 付き） |
...
- It works from the query PubMed *actually executed* and from the data *actually
  retrieved*. Grounding its output in fetched data curbs the kind of hallucination that
  stems from a model's pre-trained knowledge and probabilistic generation — every citation
  comes from a real PubMed record.
